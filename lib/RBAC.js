...
class rbac {
   ...
   * @return {object} promise
   */
  newPermission({ name, alias, sort, method }) {
    if (!name) {
    ...
    if (!method) {
      throw new Error('[egg-rbac] newPermission parameter method is undefined');
    }
    debug('new permission name %s alias %s', name, alias, sort, method);
    return this.storage.newPermission({ name, alias, sort, method });
  }
 ...
 /**
   ...
   * @return {object} promise
   */
  modifyPermissionAlias(_id, alias, sort, method) {
    if (!_id) {
    ...
    if (!method) {
      throw new Error('[egg-rbac] modifyPermissionAlias parameter method is undefined');
    }
    return this.storage.modifyPermissionAlias(_id, alias, sort, method);
  }
