...
class rbac {
  ...
  newPermission({ name, alias, sort }) {
    ...
    debug('new permission name %s alias %s', name, alias, sort);
    return this.storage.newPermission({ name, alias, sort });
  }
 ...
 /**
   ...
   * @return {object} promise
   */
  modifyPermissionAlias(_id, alias, sort) {
    if (!_id) {
    ...
    return this.storage.modifyPermissionAlias(_id, alias, sort);
  }
