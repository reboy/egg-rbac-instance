...
module.exports = exports = class mongooseStorage {
  ...
  newPermission({ name, alias, sort, method }) {
    return this.Permission.findOne({alias}).then(repeat => {
      ...
      return this.Permission.findOne({name, method}).then(repeat => {
        if (repeat) {
          throw new Error('[egg-rbac] newPermission: method repeat')
          return null
        }
        return new this.Permission({ name, alias, sort, method }).save()
      })
    })
  }
  ...
  modifyPermissionAlias(_id, alias, sort, method) {
    return this.Permission.updateOne({ _id }, { $set: { alias, sort, method } });
  }
